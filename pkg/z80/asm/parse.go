...
func bap(buf []byte, v ...any) error {
	for _, e := range v {
		binary.Append(buf, binary.LittleEndian, e)
	}
	return nil
}
...
func (t tokenIdent) token() token {
	return t
}

type tokenMnemonic string

func (t tokenMnemonic) token() token {
	return t
}

type tokenString string

func (t tokenString) token() token {
	return t
}

type tokenReg8 string

func (t tokenReg8) token() token {
	return t
}

const rbErr = 123

func (t tokenReg8) rb() uint8 {
	switch string(t) {
	case "B":
		return 0
	case "C":
...
type tokenReg16 string

func (t tokenReg16) token() token {
	return t
}
...
type tokenImm8 uint8

func (t tokenImm8) token() token {
	return t
}
...
const ErrorEmptyInstruction Error = "empty instruction not allowed"
...
func (a *Assembler) parseInstruction(tokens ...token) error {
	if len(tokens) < 1 {
		return ErrorEmptyInstruction
	}
	name, ok := tokens[0].(tokenMnemonic)
	if !ok {
		return ErrorMissingMnemonic
	}
	var operands = tokens[1:len(tokens)]

	switch string(name) {
	case "ADC":
		return a.parseADC(operands...)
	case "ADD":
...
func (a *Assembler) parseADC(operands ...token) error {
	if len(operands) != 2 {
		return ErrorNeedTwoOperands
	}
	op1, op2 := operands[0], operands[1]
	switch target := op1.(type) {
	case tokenReg8:
		if target != "A" {
			return ErrorUnexpectedRegister
		}
		return a.parseADCReg8(op2)

	case tokenReg16:
		if target != "HL" {
			return ErrorUnexpectedRegister
		}
		return a.parseADCReg16(op2)
...
func bapPtr(buf []byte, base uint8, ptr tokenPtr) error {
	regName := ptr.regName()
	switch regName {
	case "HL":
		return bap(buf, base)
...
func bapReg8(buf []byte, base uint8, reg tokenReg8) error {
	rb := reg.rb()
	if rb == rbErr {
		return ErrorUnexpectedRegister
	}
	return bap(buf, base+rb)
}

func (a *Assembler) parseADCReg8(op2 token) error {
	switch src := op2.(type) {
	case tokenPtr:
		return bapPtr(a.Binary, 0x8E, src)
	case tokenReg8:
		return bapReg8(a.Binary, 0x88, src)
	case tokenImm8:
		return bap(a.Binary, 0xCE, src)
...
func (a *Assembler) parseADCReg16(op2 token) error {
	...
	switch string(src) {
	case "BC":
		return bap(a.Binary, 0xED, 0x4A)
	case "DE":
		return bap(a.Binary, 0xED, 0x5A)
	case "HL":
		return bap(a.Binary, 0xED, 0x6A)
	case "SP":
		return bap(a.Binary, 0xED, 0x7A)
	default:
		return ErrorUnexpectedRegister
	}
...
func (a *Assembler) parseADD(operands ...token) error {
	if len(operands) != 2 {
		return ErrorNeedTwoOperands
	}
	op1, op2 := operands[0], operands[1]
	...
	case tokenReg8:
		...
		return a.parseADDReg8(op2)

	case tokenReg16:
		if target != "HL" && target != "IX" && target != "IY" {
			return ErrorUnexpectedRegister
		}
		return a.parseADDReg16(op2)
...
func (a *Assembler) parseADDReg8(op2 token) error {
	switch src := op2.(type) {
	case tokenPtr:
		return bapPtr(a.Binary, 0x86, src)
	case tokenReg8:
		return bapReg8(a.Binary, 0x80, src)
	case tokenImm8:
		return bap(a.Binary, 0xC6, src)
...
func (a *Assembler) parseADDReg16(op2 token) error {
	src, ok := op2.(tokenReg16)
	if !ok {
		return ErrorUnexpectedType
	}
	if string(src) == "IX" {
		bap(a.Binary, 0xDD)
	} else if string(src) == "IY" {
		bap(a.Binary, 0xFD)
	}

	switch string(src) {
	case "BC":
		return bap(a.Binary, 0x09)
	case "DE":
		return bap(a.Binary, 0x19)
	case "HL":
		return bap(a.Binary, 0x29)
	case "SP":
		return bap(a.Binary, 0x39)
	default:
		return ErrorUnexpectedRegister
	}
}

func isRegA(t token) bool {
	reg, ok := t.(tokenReg8)
	if !ok {
		return false
	}
...
func optionalA(operands ...token) (token, error) {
	if len(operands) == 2 {
		if !isRegA(operands[1]) {
			return nil, ErrorUnexpectedRegister
		}
...
func (a *Assembler) parseAND(operands ...token) error {
	op, err := optionalA(operands...)
	if err != nil {
		return err
	}
	switch src := op.(type) {
	...
	case tokenPtr:
		return bapPtr(a.Binary, 0xA6, src)
	case tokenReg8:
		return bapReg8(a.Binary, 0xA0, src)
	case tokenImm8:
		return bap(a.Binary, 0xE6, src)
	default:
		return ErrorUnexpectedType
	}
...
func bitOp(operands ...token) (bit uint8, op token, err error) {
	if len(operands) != 2 {
		return 0, nil, ErrorNeedTwoOperands
	}

	iv, ok := operands[0].(tokenInt)
	if !ok {
		return 0, nil, ErrorUnexpectedType
	}
	if iv < 0 || iv > 8 {
		return 0, nil, ErrorBitOutOfRange
	}
	bit = uint8(iv)
	op = operands[1]
	err = nil
...
func bapBit(buf []byte, base, regBase, bit uint8, op token) error {
	switch src := op.(type) {
	case tokenPtr:
		regName := src.regName()
		switch regName {
		case "HL":
			return bap(buf, 0xCB, base+8*bit)
		case "IX":
			return bap(buf, 0xDD, 0xCB, src.offset, base+8*bit)
		case "IY":
			return bap(buf, 0xFD, 0xCB, src.offset, base+8*bit)
		default:
			return ErrorUnexpectedRegister
		}
	case tokenReg8:
		...
		return bap(buf, 0xCB, regBase+8*bit+rb)
...
func (a *Assembler) parseBIT(operands ...token) error {
...
Collected by Oscar Lindberg 1996-03-24
